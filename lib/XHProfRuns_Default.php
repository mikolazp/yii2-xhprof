...
class XHProfRuns_Default implements iXHProfRuns {
	...
	function list_runs() {
		if (is_dir($this->dir)) {
			...
			echo '<li><a href="'.Url::toRoute('/xhprof/default/delall').'">Remove all</a></li>';
			$files = glob("{$this->dir}/*.{$this->suffix}");
			usort($files, create_function('$a,$b', 'return filemtime($b) - filemtime($a);'));
			foreach ($files as $file) {
				list($run,$source) = explode('.', basename($file));

				$url = Html::a(basename($file), ['/xhprof', 'run' => $run, 'source' => $source]);

				echo "<li>$url<small> "
					. date("Y-m-d H:i:s", filemtime($file)) . '</small>
					'.Html::a('Remove', ['/xhprof/default/del', 'run' => $run, 'source' => $source]);
			}
			echo "</ul>\n";
		}
	}
